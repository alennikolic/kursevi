...
| | |
|---|---|
| NetWorker (vault) | `networkergs.gradskacistoca.rs` |
| Vault DD | `dd-vault.gradskacistoca.rs` |
...
```bash
[root@networkergs ~]# hostname -f
networkergs.gradskacistoca.rs

[root@networkergs ~]# rpm -qa | grep -i lgto
...
[root@networkergs ~]# df -h /nsr
Filesystem             Size  Used Avail Use% Mounted on
/dev/mapper/rhel-root   70G   23G   48G  33% /

[root@networkergs ~]# ls -ld /nsr
...
```bash
[root@networkergs ~]# ssh sysadmin@dd-vault.gradskacistoca.rs
```
...
---  ---------------------------------------------  ---------  --------------------------
  1  mtree://dd-vault.gradskacistoca.rs/data/col1/nw-prod-repl  initialized  Wed Sep  9 21:40:12 2026
```
...
```
sysadmin@dd-vault# nfs add /data/col1/nw-recover 10.99.3.14
...
```bash
[root@networkergs ~]# mkdir -p /mnt/ddpeek
[root@networkergs ~]# mount -t nfs dd-vault.gradskacistoca.rs:/data/col1/nw-recover /mnt/ddpeek

[root@networkergs ~]# ls -1 /mnt/ddpeek
dd-prod_dev01
dd-prod_dev02
dd-prod_dev03

[root@networkergs ~]# ls /mnt/ddpeek/dd-prod_dev01 | head
.nsr
nwprod.gradskacistoca.rs.001
nwprod.gradskacistoca.rs.002
nwprod.gradskacistoca.rs.003
```

Sada znamo: tri uređaja, volumeni se zovu `nwprod.gradskacistoca.rs.00X`.

Odmontirati i ukloniti export — dalje idemo preko DD Boost-a:

```bash
[root@networkergs ~]# umount /mnt/ddpeek
```

```
sysadmin@dd-vault# nfs del /data/col1/nw-recover 10.99.3.14
...
```bash
[root@networkergs ~]# vi /etc/nsswitch.conf
```
...
```bash
[root@networkergs ~]# vi /etc/hosts
```

```
127.0.0.1       localhost localhost.localdomain
10.99.3.14      networkergs.gradskacistoca.rs   networkergs
10.99.3.20      dd-vault.gradskacistoca.rs      dd-vault

# Klijenti sa produkcije koji ovde nisu dostupni — na loopback,
# da server ne čeka timeout pri startu
127.0.0.1       app01.gradskacistoca.rs   app01
127.0.0.1       sql01.gradskacistoca.rs   sql01
127.0.0.1       file01.gradskacistoca.rs  file01
```

Provera:

```bash
[root@networkergs ~]# getent hosts networkergs.gradskacistoca.rs
10.99.3.14      networkergs.gradskacistoca.rs networkergs
```
...
```bash
[root@networkergs ~]# systemctl status networker | head -3
...
[root@networkergs ~]# nsradmin -s networkergs.gradskacistoca.rs
...
nsradmin> print
                        name: networkergs.gradskacistoca.rs;
                server state: active;
nsradmin> update server state: disaster recovery
                server state: disaster recovery;
Update? y
updated resource id 4.0.35.12.0.0.0.0.89.187.161.106.10.99.3.14(6)
nsradmin> quit
```
...
```bash
[root@networkergs ~]# echo -e ". type: NSR\nshow server state\nprint" | nsradmin -i -
                server state: disaster recovery;
```
...
```bash
[root@networkergs ~]# ls -l /nsr/debug/nsr_disaster_recovery_mode
...
```bash
[root@networkergs ~]# rm -f /nsr/debug/nsr_disaster_recovery_mode
```
...
```bash
[root@networkergs ~]# cat > /tmp/dddev.txt <<'EOF'
create type: NSR device;
name: dd-vault_dev01;
media type: Data Domain;
device access information: dd-vault.gradskacistoca.rs:/nw-recover/dd-prod_dev01;
remote user: ddboost;
password: <ddboost_lozinka>;
auto media management: No;
EOF

[root@networkergs ~]# nsradmin -i /tmp/dddev.txt
created resource id 174.0.49.150.0.0.0.0.89.187.161.106.10.99.3.14(2)
```
...
nsradmin> print
   ...
   device access information: dd-vault.gradskacistoca.rs:/nw-recover/dd-prod_dev01;
                     enabled: Yes;
                        ...
                        name: dd-vault_dev02;
   device access information: dd-vault.gradskacistoca.rs:/nw-recover/dd-prod_dev02;
                     enabled: Yes;
nsradmin> quit
```
...
```bash
[root@networkergs ~]# nsrmm -m -f dd-vault_dev01
Data Domain disk nwprod.gradskacistoca.rs.001 mounted on dd-vault_dev01, write enabled
```

Ako se pojavi upit tipa *"volume is not in the media database"* — to je očekivano, media baza je prazna. Potvrditi.

```bash
[root@networkergs ~]# nsrmm -C
32916:nsrmm: Data Domain disk nwprod.gradskacistoca.rs.001 mounted on dd-vault_dev01, write enabled
32916:nsrmm: Data Domain disk nwprod.gradskacistoca.rs.002 mounted on dd-vault_dev02, write enabled
32916:nsrmm: Data Domain disk nwprod.gradskacistoca.rs.003 mounted on dd-vault_dev03, write enabled
```
...
```bash
[root@networkergs ~]# scanner -v dd-vault_dev01 | head -30
scanner: scanning Data Domain disk dd-vault_dev01
scanner: ssid 3841029447: scanning session at Sep  8 22:00:14 2026
nwprod.gradskacistoca.rs: index:app01.gradskacistoca.rs  level=full   size=  412 MB  files=1  B
nwprod.gradskacistoca.rs: index:sql01.gradskacistoca.rs  level=full   size=  198 MB  files=1  BE
nwprod.gradskacistoca.rs: bootstrap                      level=full   size=  1.2 GB  files=1  BE
app01.gradskacistoca.rs:  /var/www                       level=full   size= 24 GB   files=88214 BE
sql01.gradskacistoca.rs:  D:\\SQLBACKUP                  level=full   size=310 GB   files=142   BE
```
...
```bash
[root@networkergs ~]# scanner -B dd-vault_dev01
scanner: scanning Data Domain disk dd-vault_dev01 for bootstrap save sets

  date       time      level  ssid        file  record  volume
  09/06/26   22:00:41  full   3839112204  0     0       nwprod.gradskacistoca.rs.001
  09/07/26   22:00:33  9      3840078119  0     0       nwprod.gradskacistoca.rs.001
  09/08/26   22:00:52  9      3841029447  0     0       nwprod.gradskacistoca.rs.001
...
- volumen = `nwprod.gradskacistoca.rs.001`
...
```bash
[root@networkergs ~]# date -d @$(date +%s) '+%Y-%m-%d'
2026-09-09
```
...
```bash
[root@networkergs ~]# mkdir -p /nsr/debug
[root@networkergs ~]# cat > /nsr/debug/nsrdr.conf <<'EOF'
...
Please mount the volume 'nwprod.gradskacistoca.rs.001' in device
...
completed recovery of index for client 'nwprod.gradskacistoca.rs'
completed recovery of index for client 'app01.gradskacistoca.rs'
completed recovery of index for client 'sql01.gradskacistoca.rs'
completed recovery of index for client 'file01.gradskacistoca.rs'
...
```bash
[root@networkergs ~]# nsrdr -a -B 3841029447 -d dd-vault_dev01 -I
```
...
```bash
[root@networkergs ~]# /opt/nsr/authc-server/scripts/authc_configure.sh
```
...
```bash
[root@networkergs ~]# mminfo | head
 volume                         client                    date       size    level  name
 nwprod.gradskacistoca.rs.001   app01.gradskacistoca.rs   09/08/2026  24 GB  full   /var/www
 nwprod.gradskacistoca.rs.001   sql01.gradskacistoca.rs   09/08/2026 310 GB  full   D:\SQLBACKUP
 nwprod.gradskacistoca.rs.002   file01.gradskacistoca.rs  09/08/2026 1.4 TB  full   /export/share
...

[root@networkergs ~]# mminfo -m
 state volume                         written  (%)  expires      read mounts capacity
       nwprod.gradskacistoca.rs.001   4.8 TB   72%  09/08/2027   0 KB      1     0 KB
       nwprod.gradskacistoca.rs.002   3.1 TB   48%  09/08/2027   0 KB      1     0 KB
```
...
```bash
[root@networkergs ~]# echo -e ". type: NSR client\nshow name\nprint" | nsradmin -i - | head -20
                        name: nwprod.gradskacistoca.rs;
                        name: app01.gradskacistoca.rs;
                        name: sql01.gradskacistoca.rs;
                        name: file01.gradskacistoca.rs;
```
...
```bash
[root@networkergs ~]# echo -e ". type: NSR device\nshow name; device access information\nprint" | nsradmin -i -
                        name: dd-prod_dev01;
   device access information: dd-prod.gradskacistoca.rs:/nw-prod/dd-prod_dev01;
...
nsradmin> update enabled: No
                     enabled: No;
Update? y
updated resource id 174.0.49.150.0.0.0.0.89.187.161.106.10.99.3.14(5)
nsradmin> quit
```
...
```bash
[root@networkergs ~]# echo -e ". type: NSR client; name: networkergs.gradskacistoca.rs\nshow aliases\nprint" | nsradmin -i -
                     aliases: networkergs.gradskacistoca.rs, networkergs;
```
...
```bash
[root@networkergs ~]# nsradmin -s networkergs.gradskacistoca.rs
nsradmin> . type: NSR client; name: networkergs.gradskacistoca.rs
...
[root@networkergs ~]# scanner -m dd-vault_dev01
[root@networkergs ~]# nsrck -L7 -t "09/08/2026" app01.gradskacistoca.rs

# Rezervni put — kada index backup-a nema (sporo!)
[root@networkergs ~]# scanner -i dd-vault_dev01
```
...
```bash
[root@networkergs ~]# mkdir -p /tmp/dr-test
[root@networkergs ~]# recover -s networkergs.gradskacistoca.rs \
                              -c app01.gradskacistoca.rs \
                              -d /tmp/dr-test
...
...
Received 7 file(s) from NSR server `networkergs.gradskacistoca.rs'
Recover completion time: Wed Sep  9 23:41:07 2026
recover> quit

[root@networkergs ~]# ls -R /tmp/dr-test/var/www/html
...
```bash
[root@networkergs ~]# ls -ld /nsr/res* /nsr/mm* /nsr/index*
...
```bash
[root@networkergs ~]# systemctl stop networker
[root@networkergs ~]# rm -rf /nsr/res
[root@networkergs ~]# mv /nsr/res.1757455812 /nsr/res
[root@networkergs ~]# systemctl start networker
```
...
```bash
[root@networkergs ~]# cat /nsr/logs/nsrdr.log | tail -40
[root@networkergs ~]# nsr_render_log /nsr/logs/daemon.raw | tail -40
[root@networkergs ~]# nsrwatch
```

## Za Dell Support pripremiti

```bash
[root@networkergs ~]# nsr_render_log /nsr/logs/daemon.raw > /tmp/daemon.txt
[root@networkergs ~]# cp /nsr/logs/nsrdr.log /tmp/
[root@networkergs ~]# scanner -B dd-vault_dev01 > /tmp/scanner-B.txt 2>&1
[root@networkergs ~]# nsrmm -C > /tmp/nsrmm-C.txt
[root@networkergs ~]# rpm -qa | grep -i lgto > /tmp/paketi.txt
[root@networkergs ~]# ls -ld /nsr/res* /nsr/mm* /nsr/index* > /tmp/baze.txt
```
